...
angular.module("leaflet-ng-core").directive('lfBounds', ['leafletHelpers', function (leafletHelpers) {
    return {
        ...
        link: function (scope, element, attrs, ctrl) {
            const leafletScope = ctrl.getScope(), safeApply = leafletHelpers.safeApply;
...
angular.module("leaflet-ng-core").directive('leaflet', ['$q', 'leafletData', function ($q, leafletData) {
    return {
        restrict: "EA",
        replace: true,
        scope: {lfDefaults: '=', lfLayers: '=', lfCenter: '=', lfMarkers: '=', lfBounds: '=', lfEvents: '='},
        transclude: true,
        ...
        link: function (scope, element, attrs, ctrl) {
            ...
            const map = new L.Map(element[0], scope.lfDefaults);
...
angular.module('leaflet-ng-layers').directive('lfLayers', ['leafletLayers', 'leafletData', '$q', function (leafletLayers, leafletData, $q) {
    const layerTypes = leafletLayers.getAll();
    ...
    return {
        ...
        link: function (scope, element, attrs, ctrl) {
            let leafletScope = ctrl.getScope(), layers = leafletScope.lfLayers, leafletLayers;
...
angular.module('leaflet-ng-markers').directive('lfMarkers', ['leafletData', '$q', '$log', function (leafletData, $q, $log) {
    return {
        ...
        link: function (scope, element, attrs, ctrl) {
            let leafletScope = ctrl.getScope(), markers = leafletScope.lfMarkers, leafletMarkers;
            ...
            ctrl.getMap().then(function (map) {
                ...
                leafletScope.$watch('lfMarkers', function (newMarkers) {
                    ...
                    angular.forEach(newMarkers, function (params, key) {
                        let latLng = L.latLng([params.lat, params.lng]), m;
...
const app = angular.module("app", ["leaflet-ng-core", "leaflet-ng-layers", "leaflet-ng-markers", "ngRoute", 'mgcrea.ngStrap', 'server']);
...
app.factory('DataRecord', ['mappings', '$http', '$rootScope', '$q', 'DataService', 'log', 'User', '$location', 'server', function (mappings, $http, $rootScope, $q, DataService, log, User, $location, server) {
    const _baseUrl = server.address, record = {paging: {}, current: {}};
...
app.factory('DataService', ['mappings', '$http', '$rootScope', '$q', '$timeout', 'log', 'User', '$window', '$httpParamSerializer', 'server', function (mappings, $http, $rootScope, $q, $timeout, log, User, $window, $httpParamSerializer, server) {
    let _baseUrl = server.address,
        data = {
            records: [],
            count: {},
            columns: angular.copy(mappings),
            ordering: {},
            paging: {page: 1, page_size: 200},
            busy: false,
            bounds: false,
            ndviLimits: false,
    ...
    data.download = function () {
        let params = data.getParams(), query, url;
...
app.factory('geoHelperService', [function () {
    ...
    function destination(latlon, bearing, distance) {
        let R = 6378.1, lat, lon, latDest, lonDest;
        ...
        layers: {
            ...
            overlays: {
                ...
                SouthAmerica_l3_irrigated_rainfed_2classes_1a: {
                    ...
                    params: {
                        options: {band: 'b1'},
                        url: '//tiles.croplands.org/{z}/{x}/{y}/tile.png?collection=users/croplandsdev/products&id=SouthAmerica_l3_IvsR_2classes_1a&band={band}'
                    },
...
app.factory('RatingService', ['$http', '$rootScope', 'log', 'User', '$q', 'locationFactory', 'server', function ($http, $rootScope, log, User, $q, locationFactory, server) {
    ...
    function rateRecord(record, rating) {
        ...
        $http({
            ...
            data: {record_id: record.id, rating: rating}
        }).then(function (response) {
...
app.factory('User', ['$http', '$window', '$q', 'log', '$rootScope', '$location', 'server', function ($http, $window, $q, log, $rootScope, $location, server) {
    let _user = {}, _baseUrl = server.address;
    ...
    function isLoggedIn() {
        if (_user.expires && _user.token) {
            const secondsToExpiration = _user.expires - Math.floor(new Date().getTime() / 1000);
...
app.controller("ClassifyController", ['$scope', 'mapService', 'mappings', '$http', 'leafletData', '$document', 'log', '$timeout', 'server', 'User', function ($scope, mapService, mappings, $http, leafletData, $document, log, $timeout, server, User) {
    let page = 1, minimumMapCircle, minimumMapBox, currentImageOverlay, lastClassification = new Date();
    ...
    angular.extend($scope, {
        ...
        opacity: 1,
        markers: {
            image: {layer: 'markers', lat: 0, lng: 0}
        },
        layers: {
            baselayers: {googleHybrid: angular.copy(mapService.layers.baselayers.googleHybrid)},
            overlays: {
                markers: {type: 'group', name: 'markers', visible: true}
            }
        },
        paths: {},
        buttons: [
    ...
    function drawImage() {
        ...
        minimumMapCircle = L.circle({lng: $scope.image.location.lon, lat: $scope.image.location.lat}, {
            color: '#ff0000', radius: 45, fillOpacity: 0
        }).addTo(map);
    }
...
// Function gets the next image from the array and moves the center of the map
    ...
    $scope.classify = function (classification_type) {
        ...
        if (classification_type !== undefined) {
            const data = {"classification": classification_type, "image": angular.copy($scope.image.id)};
...
app.controller("DataController", ['$scope', '$http', 'mapService', 'leafletData', function ($scope, $http, mapService, leafletData) {
    ...
    angular.extend($scope, {
        ...
        geojson: {},
        sort: {column: 'properties.crop_type', reverse: true},
    ...
    function getColor(properties) {
        ...
        let color, ratio, scale, red, green, blue;
    ...
    $scope.$watch('center', function () {
        ...
        const currentMapBounds = leafletMap.getBounds().pad(0.50);
...
app.controller("DataRecordController", ['$scope', 'mapService', 'leafletData', '$location', 'DataRecord', '$q', 'geoHelperService', function ($scope, mapService, leafletData, $location, DataRecord, $q, geoHelperService) {
    let gridImageURL = "/static/imgs/icons/grid.png",
        shapes = {};
    ...
    function buildShapes() {
        ...
        leafletData.getMap('recordMap').then(function (map) {
            ...
            shapes = {};
            const circle250 = L.circle(latlng, 125);
...
app.controller("DataSearchController", ['$scope', '$http', 'mapService', 'leafletData', '$location', 'DataService', 'DataRecord', 'leafletData', 'server', function ($scope, $http, mapService, leafletData, $location, DataService, DataRecord, leafletData, server) {
    ...
    angular.extend($scope, {
        tableColumns: [
            ...
            {id: 'lon', label: 'Longitude', visible: false},
            {id: 'lat', label: 'Latitude', visible: false},
            {id: 'land_use_type', label: 'Land Use', visible: true},
            {id: 'crop_primary', label: 'Primary Crop', visible: true},
            {id: 'crop_secondary', label: 'Secondary Crop', visible: false},
            {id: 'crop_tertiary', label: 'Tertiary Crop', visible: false},
            {id: 'water', label: 'Irrigation', visible: true},
            {id: 'intensity', label: 'Intensity', visible: true},
            {id: 'year', label: 'Year', visible: true},
            {id: 'month', label: 'Month', visible: false},
            {id: 'country', label: 'Country', visible: true},
            {id: 'source_type', label: 'Source Type', visible: true},
            {id: 'source_class', label: 'Source Class', visible: false},
            {id: 'source_description', label: 'Source Description', visible: false},
            {id: 'use_validation', label: 'Validation', visible: false}
        ],
        ordering: DataService.ordering,
        busy: true,
        bounds: {southWest: {lat: -90, lng: -180}, northEast: {lat: 90, lng: 180}},
        center: {lat: 0, lng: 0, zoom: 2},
        searchInMap: false,
        layers: angular.copy(mapService.layers),
        markers: {},
        events: {
            map: {enable: [], logic: 'emit'}
        }
    });
    ...
    function getNDVI(params) {
        let url = server.address + '/data/image?';
    ...
    function applyParams(params) {
        ...
        _.each(params, function (val, key) {
            if (DataService.columns[key] && key !== 'year' && key !== 'source_type') {
                ...
                const boundsSouthWest = val.split(',');
                DataService.bounds = {
                    southWest: {},
                    northEast: {}
                };
                ...
                const boundsNorthEast = val.split(',');
...
// Utils //
    ...
    $location.moveCenter = function (lat, lng, zoom) {
        this.search(_.merge(this.search(), {
            lat: Math.round(lat * Math.pow(10, 5)) / Math.pow(10, 5),
            lng: lng,
            zoom: zoom
        }));
    };
    ...
    $scope.setCenterLocation = function () {
        const lat = mapService.center.lat;
        const lng = mapService.center.lng;
        const zoom = mapService.center.zoom;
...
app.controller("NavbarController", ['$scope', 'User', '$location', function ($scope, User, $location) {
    $scope.goToLogin = function () {
        const n = encodeURIComponent(window.btoa(JSON.stringify({
            path: $location.path(),
...
app.controller("StreetController", ['$scope', 'mapService', 'mappings', '$http', 'leafletData', '$document', 'log', 'geoHelperService', 'server', '$timeout', function ($scope, mapService, mappings, $http, leafletData, $document, log, geoHelperService, server, $timeout) {
    ...
    angular.extend($scope, {
        defaults: {zoomControlPosition: 'bottomright'},
        center: {lat: 0, lng: 0, zoom: 2},
        layers: {
            baselayers: {googleHybrid: angular.copy(mapService.layers.baselayers.googleHybrid)},
            overlays: {
        ...
        record: {water: 0, crop_primary: 0, land_use_type: 0}
    });
    ...
    $scope.submit = function () {
        const record = angular.copy($scope.record),
            location = {
    ...
    function initMap() {
        ...
        sv.getPanoramaByLocation({lat: 14.461883, lng: 99.477472}, 50, processSVData);
    ...
    function showDirection() {
        let location = angular.copy($scope.location),
        ...
        polygon = L.polygon([original, left, right, original], {
            color: 'red',
            fillOpacity: 0,
            dashArray: '1,10'
        }).addTo(map);
...
app.controller("LoginController", ['$scope', 'log', 'User', '$timeout', '$location', function ($scope, log, User) {
    function setMessage(message, success) {
        $scope.success = success;
        $scope.message = message;
    }
    ...
    return {
        ...
        link: function (scope) {
            ...
            scope.allOptionsAreSelected = function (field) {
                ...
                for (let i = 0; i < scope[field].length; i++) {
            ...
            link: function (scope) {
                scope.$watch('pts', function () {
                    scope.points = _.map(scope.pts, function (v, i) {
                        const x = i * 52.17, y = 1000 - Math.max(3, Math.min(v, 1000));
...
app.directive('temporalBounds', ['DataService', function (DataService) {
    const bounds = {upper: [], lower: [], max: 0, min: 1000, init: false}, radius = 12;

    return {
        scope: {},
        link: function (scope, element, attributes) {
            let svg, activeBoundsInterval, activeBoundsSide, x, y, scale;
            const numIntervals = parseInt(attributes.intervals, 10);
            const intervalWidth = parseFloat(attributes.intervalWidth);
            scope.padding = 20;
                ...
                if (!bounds.init) {
                    if (DataService.ndviLimits) {
                        for (i = 0; i < numIntervals; i++) {
                            bounds.upper.push({
                                x: i * intervalWidth + scope.padding,
                                y: 1000 - DataService.ndviLimits.upper[i] + scope.padding,
                                r: radius
                            });
                            bounds.lower.push({
                                x: i * intervalWidth + scope.padding,
                                y: 1000 - DataService.ndviLimits.lower[i] + scope.padding,
                                r: radius
                            });
                        }
                    } else {
                        for (i = 0; i < numIntervals; i++) {
                            bounds.upper.push({
                                x: i * intervalWidth + scope.padding,
                                y: bounds.max + scope.padding,
                                r: radius
                            });
                            bounds.lower.push({
                                x: i * intervalWidth + scope.padding,
                                y: bounds.min + scope.padding,
                                r: radius
                            });
                        }
                    }
            ...
            scope.selectionPoints = function () {
                const upper = _.map(scope.bounds.upper, function (v) {
                    return v.x.toString() + ',' + v.y.toString();
                });
                const lower = _.map(scope.bounds.lower, function (v) {
                    return v.x.toString() + ',' + v.y.toString();
                });
...
app.directive('draggableBounds', ['$document', function ($document) {
    return {
        ...
        scope: {x: '=x', y: '=y'},
        link: function (scope, element, attributes) {
            angular.extend(scope, {radius: 20});

            const circle = element[0].children[0];
            ...
            let startY, y = angular.copy(scope.y);
